...
function handleFinanceForMaintenance(eqName, maintDesc, newCost, maintDate, maintenanceRecordId, oldCost, equipmentId) {
  return dbGetAll('finances').then(function(finances) {
    ...
    if (existingEntry) {
      if (newCost !== oldCost) {
        ...
        existingEntry.equipmentId = equipmentId;
  ...
  document.getElementById('maint-form').onsubmit = function(e) {
    ...
    dbGetAll('equipment').then(function(list) {
      ...
      if (isEdit) {
        ...
        return dbUpdate('equipment', eq).then(function() {
          return handleFinanceForMaintenance(eqName, maintDesc, maintCost, maintDate, newId, 0, equipId);
        });
      }
    }).then(function() {
      closeModal();
        ...
        return dbGetAll('equipment').then(function(list) {
          ...
          if (eq) {
            ...
            if (!existingRecord) {
              ...
              return dbUpdate('equipment', eq).then(function() {
                if (issue.cost > 0) {
                  return handleFinanceForMaintenance(eq.name, issue.desc, issue.cost, issue.resolvedAt.slice(0,10), 'issue_' + id, 0, eq.id);
                }
              });
            }
          }
